...
public m_EquipHandler(id, menu, item)
{
	...
	switch(choice)
	{
		...
		case 3:
		{
			...
			client_print(id, print_chat, "[CSDM] 在聊天栏中输入输入 ^"guns^" 来重新载入装备菜单.")
			equipUser(id, EQUIP_ALL)
		}
	}
...
{
	//Equip Menu
	menu_additem(g_EquipMenuID, "新武器", "1", 0, -1)
	menu_additem(g_EquipMenuID, "上一次的武器", "2", 0, -1)
	menu_additem(g_EquipMenuID, "不要再显示了", "3", 0, -1)
	menu_setprop(g_EquipMenuID, MPROP_EXIT, MEXIT_NEVER)
	
	//Armor Menu
	menu_additem(g_ArmorMenuID, "是, 装备护甲", "1", 0, -1)
	menu_additem(g_ArmorMenuID, "无护甲", "2", 0, -1)
	menu_setprop(g_ArmorMenuID, MPROP_EXIT, MEXIT_NEVER)
	
	//Nade Menu
	menu_additem(g_NadeMenuID, "所有手雷", "1", 0, -1)
	menu_additem(g_NadeMenuID, "无手雷", "2", 0, -1)
...
public enableMenu(id)
{
	...
	{
		g_mShowuser[id] = true
		client_print(id, print_chat, "[CSDM] 您的装备菜单已经重新启用.")
		if (!g_MenuState[id])
		{
			g_MenuState[id] = 1
		...
		client_print(id, print_chat, "[CSDM] 您的装备菜单已经启用了.")
	}
